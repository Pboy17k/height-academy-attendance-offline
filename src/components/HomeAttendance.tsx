...
export function HomeAttendance() {
  ...
  const loadRecentActivity = async () => {
    try {
      ...
      setRecentActivity(records.slice(0, 5)); // Show last 5 activities
    } catch (error) {
  ...
  const simulateFingerprintScan = async () => {
    ...
    setTimeout(async () => {
      try {
        // For demo purposes, simulate finding a staff member
        ...
        if (allStaff.length > 0) {
          ...
          // Check if within 10 minute buffer
          if (lastRecord) {
  ...
  // Admin Dashboard Layout (when logged in)
  return (
    <div className="space-y-6">
